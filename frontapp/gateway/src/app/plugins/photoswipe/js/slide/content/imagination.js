...
class ImaginationContent extends Content {
  load(/* isLazy */) {
    if (this.element) {
      return;
    }
    ...
    this.element = createElement('', 'img', this.element);
  ...
  appendImageTo(container) {
    ...
    if (this.element && !this.element.parentNode && this.isAttached) {
        let wrapper = createElement('pswp__img', 'div');
        setWidthHeight(wrapper, this.element.style.width, this.element.style.height);
