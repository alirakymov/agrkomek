...
(function(factory) {
     ...
     */
    ZoomAndPan.prototype.maxWidth = 5000;
    /**
     * hauteur maximale du canvas
     */
    ZoomAndPan.prototype.maxHeight = 5000;
    /**
    ...
    ZoomAndPan.prototype.enable = function(opt) {
        ...
        if (!hidden) {
            ...
            const downHandler = (e) => {
                ...
                pos = {
                    left: outerFrame.first().scrollLeft(),
                    top: outerFrame.first().scrollTop(),
                    x: e.clientX,
                    y: e.clientY,
                };

                outerFrame.css('cursor', 'grabbing');
                outerFrame.css('userSelect', 'none');

                JSYG(document).on('mousemove', mouseMoveHandler);
                JSYG(document).on('mouseup', mouseUpHandler);
            };

            const mouseMoveHandler = function (e) {
                const dx = e.clientX - pos.x;
                const dy = e.clientY - pos.y;
                outerFrame.scrollTop(pos.top - dy);
                outerFrame.scrollLeft(pos.left - dx);
            };

            const mouseUpHandler = function () {
                JSYG(document).off('mousemove', mouseMoveHandler);
                JSYG(document).off('mouseup', mouseUpHandler);
                outerFrame.css('cursor', '');
                outerFrame.css('userSelect', '');
            };

            outerFrame.on('mousedown', downHandler);
     ...
     */
    MouseWheelZoom.prototype.key = 'ctrl';
    /**
