...
export default class Template {
  ...
  _uiInit() {
    ...
    EventHandler.on(document, 'click', function(event) {
        ...
        const toggles = document.querySelectorAll("[data-bs-toggle='dropdown-custom']");

        for (let i = 0, len = toggles.length; i < len; i++) {
            const context = bootstrap.Dropdown.getInstance(toggles[i]);
    ...
    EventHandler.on(document, 'click', "[data-bs-toggle='dropdown-custom']", function(event) {
        event.preventDefault();

        let placement = this.getAttribute('data-popper-placement'), popperConfig = {};
        if (placement) {
