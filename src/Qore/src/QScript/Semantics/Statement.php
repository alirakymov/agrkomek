...
abstract class Statement
{
    public static function getStatements(): array
    {
        ...
        $sharedStringRegexes = [
            '_' . $labelRegex . '(?=\[)' => function(Stateful $lexer) {
                $lexer->pushState('VAR_OFFSET');

                return T_VARIABLE;
            },
            '_' . $labelRegex . '(?=->[a-zA-Z_\x7f-\xff])' => function(Stateful $lexer) {
                $lexer->pushState('LOOKING_FOR_PROPERTY');

                return T_VARIABLE;
            },
            '_' . $labelRegex => T_VARIABLE,
            '_\{(?=' . $labelRegex . '[\[}])' => function(Stateful $lexer) {
        ...
        return [ 
            ...
            'DOUBLE_QUOTES' => array_merge($sharedStringRegexes, [ 
                '"' => function(Stateful $lexer) {
            ...
            'BACKTICKS' => array_merge($sharedStringRegexes, [ 
                '`' => function(Stateful $lexer) {
                    $lexer->swapState('IN_SCRIPTING');
            ...
            'HEREDOC' => array_merge($sharedStringRegexes, []),
            'VAR_OFFSET' => [ 
                '\]' => function(Stateful $lexer) {
                    $lexer->popState();
