...
class ModelManager implements ModelManagerInterface
{
    ...
    public function __invoke($_entityName, array $_entity = null)
    {
        if (is_string($_entityName)) {
            return is_array($_entity)
                ? $this->getEntity($_entityName, $_entity)
                : (class_exists($_entityName) ? $this->getEntity($_entityName, []) : $this->getGateway($_entityName));
     ...
     */
    public function registerEntity(EntityInterface $_entity)
    {
        return $this->entityProvider->register($_entity);
    }
     ...
     * observers
     *
     * @param mixed $_observers
     */
    public function observers($_observers = [])
    {
        $container = $this->getContainer();
        foreach ($_observers as $observer => $entityClasses) {
            $observer = $container->get($observer);
            $entityClasses = ! is_array($entityClasses) ? [$entityClasses] : $entityClasses;
            foreach ($entityClasses as $entityClass) {
                if (! class_exists($entityClass)) {
                    throw new ModelManagerException(sprintf('Entity subject (%s) is not a class entity', $entityClass));
                }

                $interfaces = class_implements($entityClass);
                if (! in_array(Entity\EntityInterface::class, $interfaces)) {
                    throw new ModelManagerException(sprintf('Entity subject (%s) is not a class entity', $entityClass));
                }

                $events = ['initialize', 'create', 'save', 'delete'];
                $factors = ['before', 'after'];

                foreach ($factors as $factor) {
                    foreach ($events as $event) {
                        # - Observe $factor $event
                        $method = $factor . ucfirst($event);
                        $this->eventManager->attach(
                            $entityClass::getEventName($event, $factor),
                            $entityClass::getSafeClosure(function($_event) use ($observer, $method) {
                                if (method_exists($observer, $method)) {
                                    return $observer->$method($_event);
                                }
                            })
                        );
                    }
