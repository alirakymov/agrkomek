...
class Equal implements TypeInterface
{
    use Utils;
    ...
    {
        $this->value = $this->parseValue($_value, [Integer::class, '/.*/']);
    }
    ...
    {
        return sprintf($this->expression, $this->value);
    }
    ...
    public function apply(GatewayCursor $_where, string $_attribute): GatewayCursor
    {
        if (! is_null($this->value)) {
            $_where->equalTo($_attribute, $this->value);
        }
