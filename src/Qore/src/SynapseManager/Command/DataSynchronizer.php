...
class DataSynchronizer extends SymfonyCommand
{
     ...
     * @var string
     */
    protected static $defaultName = 'synapse:sync';
    ...
    {
        $this->setDescription('Синхронизация ORM сущностей через файловую систему')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($input->getArgument('action') === 'export') {
            ...
            if (! $config = Qore::config('qore.synapse-configs.data-synchronizer.targets', [])) {
    ...
    protected function export($_output)
    {
        ...
        foreach ($this->targets as $target => $options) {
            ...
            foreach ($objects as $object) {
                ...
                $attributes = $object->getTable()->getColumns();
        ...
        foreach ($this->targets as $target => $options) {
            ...
            file_put_contents(
                sprintf('%s/%s.php', $directory, $this->prepareTargetFileName($target)),
    ...
    protected function import($_output)
    {
        ...
        foreach ($this->targets as $target => $options) {
            ...
            $directory = $this->getNamespaceDirectory($options['namespace']);
            $targetObjects = require sprintf('%s/%s.php', $directory, $this->prepareTargetFileName($target));
            $foundObjects = [];
    ...
    {
        $targets = Qore::config('qore.synapse-configs.data-synchronizer.targets', []);
        ...
        foreach ($targets as $namespace => $names) {
            foreach ($names as $key => $value) {
                ...
                if (preg_match('/([a-z]+)\(([a-z0-9, ]*)\)/i', str_replace(' ', '', $value), $matches) )  {
                    ...
                    throw new Exception(sprintf('Can\'t parse DataSynchronizer targets %s', $value));
                }
    ...
    {
        $dumpDir = Qore::config('qore.synapse-configs.data-synchronizer.dump-dir', false);
    ...
    /**
     * prepareTargetFileName
     *
     * @param string $_target
     */
    protected function prepareTargetFileName(string $_target)
    {
        return preg_replace('/[^A-z0-9]+/', '-', $_target);
    }
