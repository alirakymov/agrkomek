...
use Qore\DealingManager\Result; use Qore\DealingManager\ResultInterface;
...
class ChainProcessor implements ScenarioClauseInterface
{
    ...
    protected function wrapEnvironment(\Closure $_closure) : ResultInterface
    {
        ...
        if ($this->model->isSearch()) {
            # - Initialize objects collection
            $this->model[Model::FILTERS_COLLECTION] = $this->collectFilters();
        }
     ...
     * @return \Iterator|null 
     */
    protected function collectFilters(): ?Iterator
    {
        ...
        return $this->model['filters']->filter( fn($_filter) => $_filter->getPath() == $path );
    }
