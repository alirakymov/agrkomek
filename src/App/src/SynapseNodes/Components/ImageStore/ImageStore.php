...
namespace Qore\App\SynapseNodes\Components\ImageStore;
 ...
 */
class ImageStore extends SynapseBaseEntity
{
    /**
     * subscribe
     *
    ...
    public static function subscribe()
    {
        ...
        static::after('initialize', function($e){
            ...
            if ( ! isset($entity['image']) && isset($entity['uniqid']) && $entity['uniqid']) {
                $entity['image'] = Qore::service(ImageManager::class)->init(
                    Qore::service(UploadManager::class)->getFile($entity['uniqid'])
                );
                $entity['imageUrl'] = $entity['image']->getUri();
            }
        });
