...
namespace Qore\App\SynapseNodes\Components\AmadeusSession\Manager;
...
class AmadeusSessionService extends ServiceArtificer
{
    ...
    public function routes(RouteCollector $_router) : void
    {
        ...
        $_router->group('/amadeus/session', null, function($_router) {
        ...
        return $this->presentAs(ListComponent::class, [
            'columns' => [
                ...
                'agentSign' => [
                    ...
                    'class-column' => 'col-2',
                    'transform' => function($_item) {
                        return htmlentities(sprintf('%s (%s)', $_item['userAlias'], $_item['agentSign']));
                    }
                ],
                'officeId' => [
                    'label' => 'Office ID',
        ...
        return [
            '{structure}' => [
                ...
                'actionUri' => function($_data) {
                    $artificer = $this->sm('AmadeusCommand:Manager');
                    return Qore::service(UrlHelper::class)->generate(
                        $this->getRouteName(get_class($artificer), 'index'),
                        [],
                        $artificer->getFilters('AmadeusSession:Connector', ['id' => $_data['id']])
                    );
                },
            ]
        ];
