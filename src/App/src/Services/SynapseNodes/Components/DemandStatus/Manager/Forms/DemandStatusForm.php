...
namespace Qore\App\SynapseNodes\Components\DemandStatus\Manager\Forms;
...
/**
 * Class: DemandStatusForm
 *
 * @see Qore\SynapseManager\Artificer\Form\FormArtificer
 */
class DemandStatusForm extends FormArtificer
{
    /**
     * subscribe
     *
        ...
        return [
            'name' => function($_field, $_entity) {
                return [
                    ...
                    [
                        'type' => Callback::class,
                        'message' => 'имя статуса должно быть уникальным',
                        'options' => [
                            'callback' => function($_value) use ($_entity) {
                                /** @var ModelManager */
                                $mm = Qore::service(ModelManager::class);
                                return is_null($mm('SM:DemandStatus')->where(
                                    fn ($_where) => $_where
                                        ->and(['@this.name' => $_value])
                                        ->notEqualTo('@this.id', $_entity->id)
                                )->one());
                            },
                        ],
                    ],
