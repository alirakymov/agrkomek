...
namespace Qore\App\SynapseNodes\Components\ProductStage\Manager;
...
/**
 * Class: ProductStageService
 *
 * @see Qore\SynapseManager\Artificer\Service\ServiceArtificer
 */
class ProductStageService extends ServiceArtificer
{
    /**
     * sortable
     *
     ...
     * @var string
     */
    private $serviceForm = 'ProductStageForm';
    ...
    public function routes(RouteCollector $_router) : void
    {
        ...
        $_router->group('/product-stage', null, function($_router) {
        ...
        return [
            'templates' => [
                ...
                'actionUri' => function($_data) {
                    $artificer = $this->sm('ProductTemplate:Manager');
                    return Qore::service(UrlHelper::class)->generate(
                        $this->getRouteName(get_class($artificer), 'index'),
                        [],
                        $artificer->getFilters('ProductStage:Connector', ['id' => $_data['id']])
                    );
                },
            ],
            'update', 'delete',
    ...
    protected function getOptionsStorage()
    {
        ...
        if (isset($this->requestFilters['__idparent'])) {
            ...
            $filter = $this->model->getFilters(true)->firstMatch([
                'namespace' => sprintf('%s.%s', $this->getNameIdentifier(), 'Product:Manager'),
            ]);

            if (isset($filter['filters']['id'])) {
                $optionsStorage = $this->mm('SM:Product')->where(function($_where) use ($filter) {
                    $filter['filters']['id']($_where, '@this.id');
                })->one();
            }
        }
