...
namespace Qore\App\SynapseNodes\Components\Product\Manager;
 ...
 */
class ProductService extends ServiceArtificer
{
    /**
     * sortable
     *
     ...
     * @var string
     */
    private $serviceForm = 'ProductForm';
    ...
    public function routes(RouteCollector $_router) : void
    {
        ...
        $_router->group('/product', null, function($_router) {
        ...
        return [
            'product-stage' => [
                ...
                'actionUri' => function($_data) {
                    $artificer = $this->sm->getServicesRepository()->findByName('ProductStage:Manager');
                    return Qore::service(UrlHelper::class)->generate(
                        $this->getRouteName(get_class($artificer), 'index'),
                        [],
                        $artificer->getFilters('Product:Manager', ['id' => $_data['id']])
                    );
                }
            ],
            'update', 'delete',
