...
namespace Qore\App\SynapseNodes\Components\OperationPhase\Manager;
 ...
 */
class OperationPhaseService extends ServiceArtificer
{
    /**
     * sortable
     *
     ...
     * @var string
     */
    private $serviceForm = 'OperationPhaseForm';
    ...
    public function routes(RouteCollector $_router) : void
    {
        ...
        $_router->group('/operation-phase', null, function($_router) {
    ...
    protected function index($_reload = false)
    {
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            return $this->response(new HtmlResponse(Qore::service(TemplateRendererInterface::class)->render('app::main', [
                'title' => 'Сервис для работы с фазами - Сервис для работы с фазами',
    ...
    protected function update()
    {
        ...
        /** @var FormMaker */
        $formMaker = $this->plugin(FormMaker::class);
        $fm = $formMaker->make('OperationPhaseCode');

        $ig = Qore::service(InterfaceGateway::class);
        $modal = $ig(Modal::class, sprintf('%s.%s', get_class($this), 'modal-update'))
            ->setTitle('Редактирование фазы')
            ->component(Qore::service(QoreFront::class)->decorate($fm))
            ->type(Modal::RIGHTSIDE)
            ->size(Modal::SIZE_XL);
        ...
        if (isset($this->requestFilters['__idparent'])) {
            $optionsStorage = $this->getLocalGateway(['id' => $this->requestFilters['__idparent']])->one();
                ...
                $filter = $this->model->getFilters(true)->firstMatch([
                    'namespace' => sprintf('%s.%s', $this->getNameIdentifier(), 'Operation:Manager'),
                ]);
                if (isset($filter['filters']['id'])) {

                    $optionsStorage = $this->mm('Operation')->where(function($_where) use ($filter) {
