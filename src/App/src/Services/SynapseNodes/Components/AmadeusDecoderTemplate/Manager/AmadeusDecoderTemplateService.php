...
namespace Qore\App\SynapseNodes\Components\AmadeusDecoderTemplate\Manager;

use Qore\DealingManager\Result;
use Qore\DealingManager\ResultInterface;
use Qore\Form\Decorator\QoreFront; use Qore\InterfaceGateway\Component\Modal;
...
class AmadeusDecoderTemplateService extends ServiceArtificer
{
     ...
     * @var string
     */
    private $serviceForm = 'AmadeusDecoderTemplateForm';
    ...
    public function routes(RouteCollector $_router) : void
    {
        ...
        $_router->group('/amadeus-decoder-template', null, function($_router) {
            $this->routingHelper->routesCrud($_router);
            $_router->any('/editor/{id:\d+}', 'template-editor');
        });
    ...
    protected function index($_reload = false)
    {
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            return $this->response(new HtmlResponse(Qore::service(TemplateRendererInterface::class)->render('app::main', [
                'title' => 'Содержимое - Работа с шаблонами
',
     ...
     */
    protected function update(): ?ResultInterface
    {
        # - Init synpase structure
        $this->next->process($this->model);
     ...
     */
    protected function templateEditor(): ResultInterface
    {
        # - Init synpase structure
        $this->next->process($this->model);
        ...
        /** @var FormMaker */
        $formMaker = $this->plugin(FormMaker::class);
        $fm = $formMaker->make('TemplateForm');

        $ig = Qore::service(InterfaceGateway::class);
        $modal = $ig(Block::class, sprintf('%s.%s', get_class($this), 'block-create'))
            ->setTitle('Редактор шаблонов')
            ->setOption('indents', true)
            ->component(Qore::service(QoreFront::class)->decorate($fm));
            ...
            'template-editor' => [
                ...
                'actionUri' => function($_data) {
                    return Qore::service(UrlHelper::class)->generate(
                        $this->getRouteName('template-editor'),
                        ['id' => $_data['id']]
                    );
                },
            ],
    ...
    protected function getOptionsStorage()
    {
        ...
        if (isset($this->requestFilters['__idparent'])) {
            ...
            $filter = $this->model->getFilters(true)->firstMatch([
                'namespace' => sprintf('%s.%s', $this->getNameIdentifier(), 'AmadeusDecoder:Manager'),
            ]);
            
            if (isset($filter['filters']['id'])) {
                $optionsStorage = $this->mm('AmadeusDecoder')->where(function($_where) use ($filter) {
