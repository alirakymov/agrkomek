...
namespace Qore\App\SynapseNodes\Components\ProductTemplate\Manager;
 ...
 */
class ProductTemplateService extends ServiceArtificer
{
    /**
     * sortable
     *
     ...
     * @var string
     */
    private $serviceForm = 'ProductTemplateForm';
    ...
    public function routes(RouteCollector $_router) : void
    {
        ...
        $_router->group('/product-template', null, function($_router) {
    ...
    protected function index($_reload = false)
    {
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            return $this->response(new HtmlResponse(Qore::service(TemplateRendererInterface::class)->render('app::main', [
                'title' => 'Управление шаблонами - Сервис управления шаблонами',
    ...
    protected function getOptionsStorage()
    {
        ...
        if (isset($this->requestFilters['__idparent'])) {
            ...
            $filter = $this->model->getFilters(true)->firstMatch([
                'namespace' => sprintf('%s.%s', $this->getNameIdentifier(), 'ProductStage:Connector'),
            ]);

            if (isset($filter['filters']['id'])) {
                $optionsStorage = $this->mm('SM:ProductStage')->where(function($_where) use ($filter) {
