...
namespace Qore\App\SynapseNodes\Components\Partner\Manager\Plugin\Report;
...
class Handler extends ReportHandler
{
    ...
    public function map(ModelInterface $_model) : bool
    {
        ...
        $attributes = $_model->getLastService()->synapse()->attributes()->filter(fn ($_attribute) => $_attribute->name !== 'emailTemplate');
     ...
     * Get attributes
     *
     * @param \Qore\SynapseManager\Plugin\Report\ModelInterface $_model
     *
     * @return \Qore\Collection\CollectionInterface
     */
    protected function getAttributes(ModelInterface $_model): CollectionInterface
    {
        $disabledAttributes = ['emailTemplate'];
        return Qore::collection(
            $_model->getLastService()->synapse()->attributes()->filter(
                fn ($_attribute) => ! in_array($_attribute->name, $disabledAttributes)
            )
        );
    }
